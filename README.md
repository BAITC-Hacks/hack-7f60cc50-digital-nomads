...
> Русская версия для быстрой проверки жюри приведена ниже; полное методологическое описание на казахском сохранено в следующих разделах.

## Проверить проект за 3 минуты
...
Откройте `http://localhost:8765`, введите любой gid и получите роль, факты, связи и разложение приоритета. Формальное объяснение в терминале:

```bash
python explain.py --gid 100000003115284100 --out out
```

Ожидаемый результат на предоставленных данных: 2 248 узлов, 3 119 рёбер, 4 840 транзакций; 2 248 строк в `nodes_roles.csv`, 45 кластеров и топ-30. Полный пересчёт занимает секунды при лимите пять минут. Сценарий защиты: [`docs/DEMO.md`](docs/DEMO.md).

### Четыре ловушки данных и наша защита

| Ловушка | Что нельзя заключать | Что делает решение |
|---|---|---|
| 444 узла на четвёртом колене без исходящих | `out_degree=0` не доказывает, что деньги осели | Модель `p_continue`, калибровка и отдельный статус `terminal (estimated)`; при AUC < 0,6 система отказывается от прогноза |
| Видны только исходящие пути от seed | Нельзя считать полный баланс клиента | `external_funding`, пропорциональная трассировка seed-денег и явная оговорка в evidence |
| У seed занижены входящие | `out/in` seed некорректен | Seed не получает роль `transit`, в evidence стоит предупреждение, приоритет seed понижен |
| Порог 5 000 KZT и только внутрибанк | Нельзя исключать дробление ниже порога или межбанк | `gaps.csv` формирует следующий запрос аналитику; выводы называются гипотезами |

### Что здесь нестандартного

- Хронологическая достижимость seed: учитывается порядок дат, поэтому статический путь не выдаётся за движение тех же денег.
- Модель обрыва обучается без признаков из невидимого будущего, калибруется и имеет безопасный режим отказа.
- Гибридный ассистент вызывает функции графа; LLM не назначает роли, а неизвестные gid помечаются как неподтверждённые.
- Устойчивость топ-N честно сравнивается с PageRank, суммой, betweenness, out-degree и случайным выбором.
