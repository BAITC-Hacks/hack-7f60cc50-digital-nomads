...
- **4-тізедегі үзілу артефактісі (`gm/boundary.py`).** 1–3 тізедегі түйіндердің шығыстары толық көрінеді. Сондықтан солардың кіріс профилі бойынша логистикалық регрессия үйретіледі. Модель «ақша әрі қарай кетті ме, әлде қалды ма» деген ықтималдықты (`p_continue`) бағалайды. Коэффициенттер есепке шығарылады. Модельдің CV ROC-AUC мәні 0.6-дан төмен болса, жүйе болжам жасамайды: барлық үзілген түйіндерге базалық үлес беріледі және бұл есепте ашық жазылады. 4-тізедегі түйін `terminal` рөлін тек «бағаланған» (`terminal_kind=estimated`) деп және төмендетілген ұпаймен ала алады.
...
| ML | scikit-learn | 4-тізедегі үзілу моделі (LogisticRegression, StandardScaler, cross_val_score) |
...
**Талаптар:** Python 3.10+ (тексерілгені 3.11), pip. GPU, бұлт немесе ақылы сервис қажет емес.
...
make test        # tests/test_pipeline.py — end-to-end тест
```
...
Пайплайн берілген датасетте (`data/`: 2 248 түйін, 3 119 қабырға, 4 840 транзакция) іске қосылды. Нәтижелер `out/report.md` және `out/run_report.json` файлдарында сақталған.
...
**Рөлдер:** `peripheral` — 1 671, `terminal` — 446 (оның 367-сі бақыланған, 79-ы бағаланған), `distributor` — 46, `consolidator` — 36, `transit` — 34, `coordinator` — 15.

**4-тізедегі үзілу моделі:** 1–3 тізедегі 1 723 түйінде үйретілді, CV ROC-AUC = 0.688. Модель бойынша үзілген 444 түйіннің шамамен 299-ы нақты «сток» болуы ықтимал.

**Кластерлер:** 45 кластер, оның 10-ында бірден көп seed бар. Табылған циклдер (қайтымды ағындар, ұзындығы ≤ 6) — 1 541.

**Тұрақтылық** — топ-N түйін алынғаннан кейін сақталған «seed → клиент» ақша жолдарының үлесі:

| Алынған түйін саны | Біздің басымдық | PageRank | Кіріс сомасы | Кездейсоқ |
|---|---|---|---|---|
| 10 | 93% | 99% | 99% | 99% |
| 20 | 61% | 96% | 98% | 98% |
| 50 | **29%** | 94% | 86% | 94% |

Яғни басымдық тізіміндегі топ-50 түйін алынса, ақша жолдарының 71%-ы үзіледі. PageRank немесе сома бойынша таңдағанда бұл көрсеткіш 6–14% ғана.

**Толық есептеу уақыты:** ~6 секунд (әзірлеу ортасында, `viewer.html` құрумен қоса).
...
|---|---|---|
| `consolidator` | `in_deg ≥ 5` әр түрлі төлеуші **және** `in_kzt ≥ P75` (кіріс алушылар арасында) | төлеушілер саны, кіріс перцентилі, seed-төлеушілер |
...
| `terminal` | `out_deg = 0` **және** `depth < 4` (бақыланған) **немесе** 4-тізе және `p_continue < 0.35` (бағаланған); әрі `in_kzt ≥ 100 000` немесе `in_deg ≥ 2` | бақыланған: 0.6 + 0.4·перцентиль; бағаланған: 0.6·(1−p) |
| `coordinator` | `seed_reach ≥ 5`, `in_kzt ≥ P97`, түйін «науа» емес (бүкіл ақшаны бір алушыға өткізбейді), және мына шарттардың кем дегенде біреуі орындалады: ≥2 төлеушісі consolidator/coordinator; distributor-ды қаржыландырады; ≥2 агрегат-тармақты біріктіріп, кез келгенінен ≥5 seed артық жинайды (`merge_gain`); бір мезгілде жинайды әрі таратады | seed_reach, кіріс перцентилі, орындалған критерийлер саны |
| `peripheral` | қалған түйіндер | рөл белгілерінің **жоқтығына** сенімділік; 4-тізеде ×0.6 |

**Негізгі метрикалар:**

- `seed_reach` — түйінге бағытталған жол арқылы жететін әр түрлі seed-тер саны.
- `merge_gain` — түйін ең «бай» төлеушісінен артық қанша seed-ағынды біріктіреді. Дистрибьютордың дропы үшін бұл 0, консолидатор үшін — оның seed-тері саны.
...
- **Parquet-ті pyarrow арқылы оқу әзірлеу ортасында тексерілмеген.** Ортада pyarrow болмады, сондықтан берілген `.parquet` файлдары DuckDB CLI көмегімен сол бағандары бар `.csv` файлдарына түрлендірілді, ал пайплайн оларды `io.py` ішіндегі CSV жолы арқылы оқыды. Қалыпты машинада `requirements.txt` ішіндегі pyarrow арқылы `.parquet` тікелей оқылады (`pandas.read_parquet`).
...
- **LLM режимі нақты API кілтімен тексерілмеген.** Детерминистік режим және HTTP API тексерілген. LLM шақыру коды (OpenAI-мен үйлесімді / Anthropic) бар, бірақ қате болса, көмекші автоматты түрде детерминистік режимге ауысады.
...
- **Үзілу моделінің сапасы орташа.** Нақты деректерде CV ROC-AUC = 0.688, синтетикада ≈ 0.59 болды (бұл жағдайда жүйе болжам жасамай, базалық үлесті қолданды). Сондықтан `terminal (estimated)` рөлі тек бағалау болып табылады.
